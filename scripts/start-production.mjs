...
function run(args) {
  return new Promise((resolvePromise, reject) => {
    const child = spawn(pnpm, args, { stdio: 'inherit', env: environment });
...
try {
  ...
  const api = spawn(pnpm, ['--filter', 'caoa-venda-cantada-api', 'start'], {
    stdio: 'inherit',
    env: environment
  });
  ...
  await waitForApi();
  const web = spawn(pnpm, ['--filter', 'caoa-venda-cantada-web', 'start'], {
    stdio: 'inherit',
    env: environment
  });
